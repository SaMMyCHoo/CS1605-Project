...
void print(ostream &os,int side,operation op,Pet& x,Pet& y)
{
    ...
    else
    {
        ...
        {
            os<<"Enemy uses Revival Potion on ";
            if(op.obj==1)
                os<<"W";
            else if(op.obj==2)
                os<<"L";
            ...
            os<<"Enemy uses Attack Potion on "<<enemy.UP_str()<<endl;
    }
}
void carry(operation op,Pet& x,Pet& y)
{
...
void play(ostream &os,istream &is)
{
    int HP_drag=1,ATK_drag=2;
    int round=0;
    while(true)
    {
        os<<"--------------------------------------------------"<<endl;
        os<<"Round "<<++round<<endl;
        operation now,Enow;//利用结构体存储玩家与电脑的操作，便于统一。choice是操作类型，obj是操作对象
        while(true)
        {
        ...
        //敌方决策
        if(enemy.deadcnt()>0&&HP_drag>0)//第一次运行到这里时可以保证只有一个宠物死亡
        {
            --HP_drag;
            Enow.choice=2;
            for(int i=1;i<=3;++i)
        ...
        else if(!player.advantage(enemy.get_UP())&&ATK_drag>0&&!enemy.draged())//未被我方克制
        {
            --ATK_drag;
            Enow.choice=3;
            Enow.obj=enemy.get_UP();
        }
        else if(enemy.draged())//不能主动换下嗑药的
        {
            Enow.choice=1;
            if(enemy.advantage(player.get_UP()))
                Enow.obj=2;
            else
                Enow.obj=1;
        }
        else 
        {
            ...
            {
                int best=player.get_UP()%3+1;
                if(enemy.alive(best))
                    Enow.choice=0,Enow.obj=best;
                else//此时已经不存在克制敌人的宠物了
                {
                    if(enemy.get_UP()==player.get_UP())//未被克制
                        Enow.choice=1,Enow.obj=1;
                    else//被克制    
                    {
                        if(enemy.alive(player.get_UP()))//能换就换
                            Enow.choice=0,Enow.obj=player.get_UP();
                        else//没办法了，只能硬刚
                            Enow.choice=1,Enow.obj=1;
                    }
                }
