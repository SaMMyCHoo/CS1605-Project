...
void init(ostream &os,istream &is)
{
    ...
    os<<"Your "<<player.pets[0].get_name()<<": HP "<<player.pets[0].get_hp()<<" || Enemy’s D: HP 120"<<endl;
...
void play(ostream &os,istream &is)
{
    ...
    while(1)
    {
        ...
        if(enemy.up==0)
        {
            if((player.pets[player.up].get_hp()*3<player.pets[player.up].get_maxhp())&&(player.pets[player.up].asleep()||player.pets[player.up].painted()))
                C_c.choice=1,C_c.obj=0;
            else if(!player.pets[player.up].asleep())
                C_c.choice=1,C_c.obj=1;
            else if(!player.pets[player.up].painted())
        ...
        {
            if(enemy.pets[1].get_hp()<55&&enemy.pets[2].get_hp()>0)
                C_c.choice=2,C_c.obj=2;//换成小龙
            else
            {
                C_c.choice=1;
                if(enemy.pets[1].get_hp()>55)
                {
                    if(player.pets[player.up].get_Type()==3)
                        C_c.obj=1;
                    else
                        C_c.obj=0;
                }
                ...
                {
                    if(enemy.pets[1].get_hp()>player.pets[player.up].get_hp())
                    {
                        if(player.pets[player.up].get_Type()==3)
                            C_c.obj=1;
                        else
        ...
        {
            C_c.choice=1;
            if(player.pets[player.up].get_hp()*5<player.pets[player.up].get_maxhp()&&(player.pets[player.up].get_Type()==2||player.pets[player.up].get_Type()==3))
                C_c.obj=0;
            else
            {
                if(weather>0)
